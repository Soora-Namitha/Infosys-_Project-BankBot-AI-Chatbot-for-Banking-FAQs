...
# Train a DistilBERT intent classifier on intents.json
...
def train(args):
    ...
    train_txt, val_txt, train_lbl, val_lbl = choose_train_test_split(
        texts, labels, default_frac=0.2
    )
    ...
    with open(
        os.path.join(args.output_dir, "label2id.json"), "w", encoding="utf-8"
    ) as f:
        json.dump(label2id, f)

    with open(
        os.path.join(args.output_dir, "id2label.json"), "w", encoding="utf-8"
    ) as f:
